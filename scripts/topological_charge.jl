...
function full_observable_from_hdf5(file; label, name, plot_label, outdir = "./output/plots",  therm = 1)
    ...
    f = h5open(file)
    for ens in keys(f)
        cfgn, obs = read(f[ens],"trajectories"), read(f[ens],name)
                
        plt1,τmax,τexp = MadrasSokal.publication_plot(cfgn,obs,plot_label,therm)
